...
# Set up logger
logger = CustomLogger("experiment_hr_nn", log_to_local=False)
...
data = load_data(file_path_list=[PROJECT_DIR / "data/train/power/power-hr-train.tsv"],text_head="text")
...
write_ndjson_file(results_NeuralNetwork_word, RESULTS_DIR / "results_hr_NeuralNetwork_word.json")
...
write_ndjson_file(results_NeuralNetwork_char, RESULTS_DIR / "results_hr_NeuralNetwork_char.json")
...
write_ndjson_file(results_aggr, RESULTS_DIR / "results_hr_NeuralNetwork.json")

results_df = pd.DataFrame(data=results_aggr)
results_df.to_csv(RESULTS_DIR / "results_hr_NeuralNetwork.csv", index=False)
results_df.to_latex(RESULTS_DIR / "results_hr_NeuralNetwork.tex", index=False)