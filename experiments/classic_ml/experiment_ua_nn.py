...
# Set up logger
logger = CustomLogger("experiment_ua_nn", log_to_local=False)


# Setup folders

MODELS_CACHE_DIR = PROJECT_DIR / ("models/gb")
...
write_ndjson_file(results_NeuralNetwork_word, RESULTS_DIR / "results_ua_NeuralNetwork_word.json")
...
write_ndjson_file(results_NeuralNetwork_char, RESULTS_DIR / "results_ua_NeuralNetwork_char.json")
...
write_ndjson_file(results_aggr, RESULTS_DIR / "results_ua_NeuralNetwork.json")

results_df = pd.DataFrame(data=results_aggr)
results_df.to_csv(RESULTS_DIR / "results_ua_NeuralNetwork.csv", index=False)
results_df.to_latex(RESULTS_DIR / "results_ua_NeuralNetwork.tex", index=False)