...
# Set up logger
logger = CustomLogger("results_ua_XGBoost", log_to_local=False)
...
# Train, test, evalute
model_XGBoost_word, results_XGBoost_word = train_evaluate_xgboost(data, NFOLDS, "word", logger, PREFERRED_DEVICE)
...
write_ndjson_file(results_XGBoost_word, RESULTS_DIR / "results_ua_XGBoost_word.json")
...
# Train, test, evalute
model_XGBoost_char, result_XGBoost_char = train_evaluate_xgboost(data, NFOLDS, "char", logger, PREFERRED_DEVICE)
...
write_ndjson_file(result_XGBoost_char, RESULTS_DIR / "results_ua_XGBoost_char.json")
...
write_ndjson_file(results_aggr, RESULTS_DIR / "results_ua_XGBoost.json")

results_df = pd.DataFrame(data=results_aggr)
results_df.to_csv(RESULTS_DIR / "results_ua_XGBoost.csv", index=False)
results_df.to_latex(RESULTS_DIR / "results_ua_XGBoost.tex", index=False)
